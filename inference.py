...
from odourdataset import OdourDataset_train, OdourDataset_val, OdourDataset_test, OdourDataset_eval
...
save_eval = '/content/drive/MyDrive/dgl_hgp_sl/eval_json'

if not os.path.exists(save_weights):
    os.makedirs(save_weights)
if not os.path.exists(save_eval):
    os.makedirs(save_eval)
...
def test(model: torch.nn.Module, loader, device, num_classes, e):
    ...
    hi_da = 0
    for batch in loader:
        batch_graphs, batch_labels, batch_chems, batch_iupac = batch
        ...
        if e % 10 == 0 and hi_da == 0:
            ...
            for iss in range(len(pred_list)):
                eval_json.append({'iupac': batch_iupac[iss], 'label_list': label_list[iss], 'pred_list': pred_list[iss],
                                  'value_list': vallist[iss]})
...
def main(args):
    ...
    inf_loader = GraphDataLoader(eval_set, batch_size=args.batch_size, shuffle=True, num_workers=2)
    ...
    num_n_feature, num_e_features, num_classes, num_graphs = eval_set.statistics()
    ...
    eval_acc, _, eval_loss = test(model, inf_loader, device, num_classes, e)
    print ("SAVING JSON...")

    json.dump(eval_json, open(os.path.join(save_eval, 'eval.json'),'w',  encoding='utf8'), ensure_ascii = False)
