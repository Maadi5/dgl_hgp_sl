...
class OdourDataset_train(DGLDataset):
    ...
    def process(self):
        ...
        df = pd.read_csv(os.path.join(savepath, ('odour_graphs_t10_' + str('train') + '.csv')))
        self.graphs = []
        self.labels = []
        self.labels_set = set()
...
class OdourDataset_val(DGLDataset):
    ...
    def process(self):
        savepath = '/content/drive/MyDrive/dgl_hgp_sl/dataset/'
        df = pd.read_csv(os.path.join(savepath, ('odour_graphs_t10_' + str('valid') + '.csv')))
        self.graphs = []
        self.labels = []
        self.labels_set = set()
        for idx, row in df.iterrows():
...
class OdourDataset_test(DGLDataset):
    ...
    def process(self):
        savepath = '/content/drive/MyDrive/dgl_hgp_sl/dataset/'
        df = pd.read_csv(os.path.join(savepath, ('odour_graphs_t10_' + str('test') + '.csv')))
        self.graphs = []
        self.labels = []
        self.labels_set = set()
        for idx, row in df.iterrows():
